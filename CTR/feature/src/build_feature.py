...
#  获得特征数据
#  输入：系统相关的数据表   word_file：关键词字典文件   category_file:类别词典  source_file:视频来源
#  输出：特征向量  什么格式？？vsm
######################################################################
import json
import time
...
def load_feature_id(word_file="idf.dict", category_file="category.dat", source_file="source.dat"):
    feature_id = 1
    #用户特征,关键词特征id
    input_file = open(word_file, 'r')#idf.dict 格式 ：词  词的idf值     例如： 1号店	13.9802
    for line in input_file:
        line = line.strip()
        item_list = line.split()
        word = "uw_" + item_list[0]#词
        feature_name_id_dict[word] = feature_id
        feature_id += 1
    input_file.close()

    # 用户特征,类别特征id     category.dat
    #==================
    #2   娱乐八卦
    #4   影视剧吐槽
    #===================
    ...
    for line in input_file:
        line = line.strip()
        item_list = line.split()
        category = "uc_" + item_list[1]#类别名
        feature_name_id_dict[category] = feature_id
        feature_id += 1
    input_file.close()

    #用户特征,用浏览数
    feature_name_id_dict["uv"] = feature_id# user view
    feature_id += 1

    #视频特征，关键词特征id
    input_file = open(word_file, 'r') #word_file="idf.dict"
    for line in input_file:
        line = line.strip()
        item_list = line.split()
        word = "vw_" + item_list[0] #词典的词
        feature_name_id_dict[word] = feature_id
        feature_id += 1
    input_file.close()
...
这个函数是此模块的核心，需要从数据库中抽取相关的特征，构建特征向量
"""
def get_vsm(action):
    uid = action["uid"]
    ...
    for word in user_keyword_list:
        feature_name = ("uw_" + word).encode("utf-8")
        if feature_name_id_dict.has_key(feature_name):
    ...
    for category in user_category_list:
        feature_name = ("uc_" + category).encode("utf-8")
        if feature_name_id_dict.has_key(feature_name):
            feature_id = feature_name_id_dict[feature_name]
            vsm.append( [feature_id,1] )
    ...
    for word in video_keyword_list:
        feature_name = ("vw_" + word).encode("utf-8")
        if feature_name_id_dict.has_key(feature_name):
            feature_id = feature_name_id_dict[feature_name]
            vsm.append( [feature_id,1] )
    ...
    for category in video_category_list:
        feature_name = ("vc_" + category).encode("utf-8")
        if feature_name_id_dict.has_key(feature_name):
            feature_id = feature_name_id_dict[feature_name]
            vsm.append( [feature_id,1] )
...
def build_unview_action():
    ...
    for action in action_list:
        ...
        feature_id_value_list = sorted(vsm, key=lambda d: d[0], reverse=False)#lambda表达式
