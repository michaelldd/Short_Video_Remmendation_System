...
def get_user_actions():
    #获得未处理的用户行为记录
    sql_com = "select * from user_action_ctr where is_processed=0"
    result = sql_appbk.mysql_com(sql_com)

    #更新已处理的记录
    sql_com = "UPDATE user_action_ctr set is_processed = 1 WHERE is_processed=0"
...
def get_video_info(vid):
    sql_com = "select id,title from video_info_ctr where id =" + str(vid)
    result = sql_appbk.mysql_com(sql_com)
    if result:
        return result[0]
    else:
...
def update_db(uid, appbk_new_sub_category, appbk_new_tags):
    ...
    sql_com = "replace into user_tags_ctr (uid ,appbk_sub_category ,appbk_tags,update_time)\
            VALUES('" + str(
...
def update_user_model():
    ...
    for action in action_list:
        ...
        appbk_new_sub_category = combine(classes, appbk_sub_category)
        appbk_new_tags = combine(keywords, appbk_tags)
